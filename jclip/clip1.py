...
from .model_res import build_model_res
from .model1 import build_model
from .simple_tokenizer import SimpleTokenizer as _Tokenizer
from PIL import Image
from jittor.transform import CenterCrop, ImageNormalize, Compose, _setup_size, to_pil_image, resize,ToTensor, RandomHorizontalFlip
...
def load(name, download_root=None, mode='vit'):
    if name in _MODELS:
    ...
    if mode=='vit':
        ...
        model = build_model_res(state_dict)
    return model, _transform1(model.visual.input_resolution), _transform2(model.visual.input_resolution), tfm_train_base(model.visual.input_resolution),tfm_train_base1(model.visual.input_resolution)
    
def load_vlp(name, download_root=None, mode='vit'):
    if name in _MODELS:
        model_path = _download(
            _MODELS[name], download_root
            or os.path.expanduser("~/.cache/clip"))
