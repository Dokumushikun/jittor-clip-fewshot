...
def cls_acc(output, target, topk=1):
    ...
    # 条件1：预测和标签都在 373 及之前
    correct_1 = (pred < 373) & (target.view(1, -1) < 373)
    
    # 条件2：预测和标签都在 374 及之后
    correct_2 = (pred >= 373) & (target.view(1, -1) >= 373)
    
    # 满足其中之一即为正确
    correct = (correct_1 | correct_2).expand_as(pred)
...
def split_ood(args, clip_model, loader):
    clip_model.eval()
    with jt.no_grad():
        template = load_class_names('text_template')
        textual_features = clip_classifier(template, clip_model).squeeze(0).t()
        ...
        acc2 = 0.
        tot_samples = 0
        with open('Dataset/TestSetB_1.txt', 'w') as f1, open('Dataset/TestSetB_2.txt', 'w') as f2:
            for i, (image, images, target, impath) in enumerate(tqdm(loader)):
                image = jt.array(image)
                images = jt.array(images)
                image = image.squeeze(1)
                ...
                pred = cosine_similarity_mta.argmax(dim=-1)[0].numpy()

                for j in range(len(impath)):
                    if pred[j] <= 372:
                        f1.write(impath[j] + '\n')
                    else:
                        f2.write(impath[j] + '\n')
...
def main():
    ...
    clip_model.eval()
    split_ood(args, clip_model, test_loader)
