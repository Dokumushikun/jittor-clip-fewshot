...
def save_lora(args, epoch, list_lora_layers):
    ...
    save_path = f'test_pkl/lora_weights.pkl'
...
def evaluate_lora(args,epoch, all_image_features_zs, moco_adapter, moco_model, channel_lp, prompt_learner, Text_encoder, clip_model, valid_loader, test_loader, clip_model_zs):
    clip_model.eval()
    channel_lp.eval()
    prompt_learner.eval()
    moco_model.eval()
    ...
    with jt.no_grad():
        ...
        for i, (image, images, target, impath, index) in enumerate(tqdm(test_loader)):
            ...
            for idx in range(1):
                ...
                results4.append(f"{filename} {top5_str4}")
                
                top5_labels5 = top5_pred5[idx].tolist()  
                top5_str5 = ' '.join(map(str, top5_labels5)) 
                results5.append(f"{filename} {top5_str5}")

                top5_labels6 = top5_pred6[idx].tolist()  
                top5_str6 = ' '.join(map(str, top5_labels6)) 
                results6.append(f"{filename} {top5_str6}")
                
                top5_labels7 = top5_pred7[idx].tolist()  
                top5_str7 = ' '.join(map(str, top5_labels7)) 
                results7.append(f"{filename} {top5_str7}")


        with open(f'final_results/{epoch}_top5_results4.txt', 'w') as f:
            for result in results4:
                f.write(result + '\n')
        with open(f'final_results/{epoch}_top5_results5.txt', 'w') as f:
            for result in results5:
                f.write(result + '\n')
        with open(f'final_results/{epoch}_top5_results6.txt', 'w') as f:
            for result in results6:
                f.write(result + '\n')   
        with open(f'final_results/{epoch}_top5_results7.txt', 'w') as f:
            for result in results7:
                f.write(result + '\n')  
        '''

        for i, (image, images, target, impath, index) in enumerate(tqdm(valid_loader)):
            image = jt.array(image)
            images = jt.array(images)
            image = image.squeeze(1)
            images = images.squeeze(1)
...
def run_lora(args, trainset_feature_loader, clip_model_zs, clip_model, logit_scale, dataset, train_loader,moco_loader_feature, val_loader, test_loader):
    # Textual features
    ...
    load_lora(args, list_lora_layers, 'lora_weights1/lora_weights.pkl')
    clip_model = clip_model.eval()

    list_lora_layers1 = apply_lora(args, clip_model_zs)
    load_lora(args, list_lora_layers1, 'lora_weights1/lora_weights.pkl')
    clip_model_zs = clip_model_zs.eval()

    #zs_clip_features, _ = pre_load_zs(clip_model_zs, trainset_feature_loader)
    zs_clip_features = jt.load('features_zs1.pkl')

    zs_clip_features = jt.array(zs_clip_features)
    zs_clip_features = zs_clip_features.squeeze(1)
    print(zs_clip_features.shape)
    ...
    #prompts = prompt_learner()    
    #text_features_pt = Text_encoder(prompts, tokenized_prompts)
    
    textual_features = clip_classifier(template, clip_model_zs)

    #test_features, test_labels = pre_load_zs1(clip_model_zs, val_loader)
    test_features = jt.load('features_zs2.pkl')
    test_labels = jt.load('label_zs2.pkl')
    ...
    #textual_features = textual_features.squeeze(0)
    channel_lp.fc.weight.update(textual_features)
    channel_lp = channel_lp.cuda()

    moco_adapter = Moco_Adapter()
    moco_features, moco_labels = pre_load_features_moco(moco_model, moco_loader_feature)
    #moco_adapter.fc.weight.update(textual_features)
    init_weight = jt.zeros((2048, 403))

 
    for i in range(moco_features.shape[0]):
        label_index = int(moco_labels[i])  
        init_weight[:, label_index] += moco_features[i]
    moco_adapter.fc.weight.assign(jt.transpose(init_weight, (1, 0)))
    moco_adapter = moco_adapter.cuda()
    ...
    #Prompt_parameters = prompt_learner.parameters()
    Prompt_parameters = get_prompt_parameters(prompt_learner)
    inter_parameters = get_vlt_parameters(clip_model)
    channel_lp_parameters = channel_lp.parameters()
    moco_adapter_parameters = moco_adapter.parameters()
    print(len(Prompt_parameters), len(inter_parameters), len(lora_parameters), len(channel_lp_parameters), len(moco_adapter_parameters))
    all_parameters = list(Prompt_parameters) + list(inter_parameters) + list(lora_parameters) + list(channel_lp_parameters) + list(moco_adapter_parameters)
    #all_parameters = list(Prompt_parameters) + list(inter_parameters) + list(channel_lp_parameters)
    #all_parameters = list(Prompt_parameters) + list(inter_parameters) + list(lora_parameters)
    #all_parameters = list(Prompt_parameters) + list(inter_parameters) 

    optimizer = jt.optim.AdamW(all_parameters, weight_decay=1e-2, betas=(0.9, 0.999), lr=args.lr)
    ...
    with jt.no_grad():
        text_features_zs = clip_classifier(template, clip_model_zs) 
        text_features_zs = text_features_zs.squeeze(0)
        text_features_zs_trains = []
        for idx in range (1, 9):
            template_random = load_class_names_random('text_template', idx)
            text_features_zs_train = clip_classifier(template_random, clip_model_zs) 
            text_features_zs_train = text_features_zs_train.squeeze(0)     
            text_features_zs_trains.append(text_features_zs_train)
        text_features_zs_trains = jt.array(text_features_zs_trains)
    ...
    while epoch < total_epoch:
        ...
        for (images, target, index) in tqdm(train_loader):
            ...
            #######templates#######
            #with jt.no_grad():
            #    text_features_tem = clip_classifier(template, clip_model).squeeze(0)
            ##########templates#########
            ...
            #text_features = (text_features + text_features_zs) / 2
            #text_features = text_features / text_features.norm(dim=-1, keepdim=True)

            # Compute the prompted logits


            cosine_similarity = 100 * image_features @ text_features.t()
            #print(image_features)
            #print(target)

            # Now calculate the frozen pre-trained features
            #fixed_embeddings = prompt_learner.fixed_embeddings  # precomputed pre-trained frozen textual features
            #fixed_embeddings = fixed_embeddings / fixed_embeddings.norm(dim=-1, keepdim=True)

            with jt.no_grad():
                #zero_shot_features = clip_model_zs.encode_image(tfm_clip(images))
                #zero_shot_features = zero_shot_features / zero_shot_features.norm(dim=-1, keepdim=True)
                # Compute pre-trained frozen visual features
                #zero_shot_logits = 100 * zero_shot_features @ fixed_embeddings.t()
                zero_shot_logits = 100 * zs_clip_features[index, :] @ text_features_zs.t()
                
            loss_scl_text = nn.l1_loss(text_features, text_features_zs) 
            loss_scl_image = nn.l1_loss(image_features, zs_clip_features[index, :])
            a = nn.log_softmax(cosine_similarity / 1, dim=1) 
            b = nn.log_softmax(zero_shot_logits  / 1, dim=1)
            L_SCL_logits = kl_div(a, b, reduction='sum') * (1 * 1) / cosine_similarity.numel()

            with jt.no_grad():
                image_features_ = clip_model.encode_image(tfm_clip(images))
                text_features_zs_train = random.choice(text_features_zs_trains)
            features = jt.concat((image_features_, text_features_zs_train), dim=0)
 
            outputs_lp = channel_lp(features)
            outputs_lp = logit_normalize(outputs_lp)
            target_text = jt.arange(0, 403)
            target_repeat = jt.concat((target, target_text))
            lp_ce = nn.cross_entropy_loss(outputs_lp, target_repeat)

            aux_logits = moco_model(tfm_moco(images))
            out_moco = moco_adapter(aux_logits)
            out_moco = logit_normalize(out_moco)
            loss_aux = nn.cross_entropy_loss(out_moco, target)

            #loss = 0.35 * loss_merge + 0.65 * loss_aux
            L_SCL = (L_SCL_logits + loss_scl_text + loss_scl_image)

            sim_ce = nn.cross_entropy_loss(cosine_similarity, target)

            #loss = sim_ce + L_SCL + 0.5*(lp_ce + loss_aux)
            loss = sim_ce + L_SCL + (lp_ce + loss_aux)
            #loss = sim_ce + (lp_ce + loss_aux)
